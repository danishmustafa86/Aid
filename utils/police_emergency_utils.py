...
from agents.police_emergency_agent import police_emergency_graph
...
def load_history(user_id: str):
    ...
    try:
        ...
        raise Exception(f"Error loading police emergency chat history: {e}")
...
def save_history(user_id: str, user_message: str, bot_messages: str):
    ...
    try:
        ...
        raise Exception(f"Error saving police emergency chat history: {e}")

async def respond(user_id: str, user_message: str):
    try:
        config = {"configurable": {"thread_id": f"police_emergency_{user_id}"}}
        combined_response = ""
        for step in police_emergency_graph.stream(
        ...
        raise Exception(f"Error generating police emergency response: {e}")
