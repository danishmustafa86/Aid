...
checkpoint_saver = MongoDBSaver(
    ...
    db_name="aidlinkai_checkpoints"
)

async def respond(user_id: str, message: str) -> str:
    """Respond to user message using the fire emergency agent"""
    try:
        # Create a thread ID for this user
        thread_id = f"fire_emergency_{user_id}"
        
        # Create the graph with checkpoint
        graph = create_fire_emergency_graph()
        ...
        return "I'm here to help with your fire emergency. Please describe the situation."
        
    except Exception as e:
        logger.error(f"Error in fire emergency respond function: {e}")
        return "I'm experiencing technical difficulties. Please try again or contact emergency services directly."
...
async def save_history(user_id: str, user_message: str, bot_response: str):
    ...
    try:
        ...
        # Check if user exists in chat collection
        user_record = chat_collection.find_one({"user_id": thread_id})
        ...
        logger.error(f"Error saving fire emergency chat history: {e}")
