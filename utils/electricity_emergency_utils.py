...
from agents.electricity_emergency_agent import electricity_emergency_graph
...
def load_history(user_id: str):
    ...
    try:
        ...
        raise Exception(f"Error loading electricity emergency chat history: {e}")
...
def save_history(user_id: str, user_message: str, bot_messages: str):
    ...
    try:
        ...
        raise Exception(f"Error saving electricity emergency chat history: {e}")

async def respond(user_id: str, user_message: str):
    try:
        config = {"configurable": {"thread_id": f"electricity_emergency_{user_id}"}}
        combined_response = ""
        for step in electricity_emergency_graph.stream(
        ...
        raise Exception(f"Error generating electricity emergency response: {e}")
